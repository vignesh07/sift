...
function getReasonLabel(item: Item): string | null {
  ...
  if (reasons.includes('issue_on_owned_repo')) return 'opened an issue on your repo';
  if (reasons.includes('your_pr_changes_requested')) return 'changes requested';
  if (reasons.includes('your_open_pr')) return 'your PR';
  if (reasons.includes('your_open_issue')) return 'your issue';
  // Layer 2 / Layer 3
  if (reasons.includes('maintainer_on_owned_repo')) return 'maintainer';
  if (reasons.includes('maintainer_on_contributing_repo')) return 'maintainer';
  // Layer 4
...
export default function ItemRow({ item, emphasized = false }: ItemRowProps) {
  const baseCfg = LAYER_CONFIG[item.layer] ?? LAYER_CONFIG[5];
  const cfg = emphasized && item.layer > 1
    ? { ...baseCfg, ...EMPHASIZED_ROW_CONFIG, paddingBlock: baseCfg.paddingBlock }
    : baseCfg;
  ...
  return (
    ...
    >
      ...
      {/* Chevron */}
      {showChevron && (
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={cfg.chevronColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: 4, transition: 'stroke 180ms ease' }}>
